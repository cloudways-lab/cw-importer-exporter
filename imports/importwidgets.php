<?php
class CloudwaysImporter_importwidgets {

    private $settings;

    public function __construct($settings)
    {
        if ((isset($settings['sidebar_id']) && $settings['sidebar_id'])
            && (isset($settings['widget_instance_id']) && $settings['widget_instance_id'])
            && (isset($settings['widget_name']) && $settings['widget_name'])
            ) {
            $this->settings = [ $settings['sidebar_id'] =>[ $settings['widget_instance_id'] => $settings['widget_name']]];     
        }
        // $this->settings = ['sidebar-1'=>['recent-comments'=> 'Recent Comments']];
        $this->process();
    }
    ...
    public function process() {
		...
		WP_CLI::success( $count.' Widgets imported successfully.' );
